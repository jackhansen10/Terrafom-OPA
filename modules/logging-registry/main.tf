...
locals {
  ...
  key_account_region = sprintf("%s:%s", [local.account_id, local.region])
  key_account_any    = sprintf("%s:*",   [local.account_id])
  key_any_region     = sprintf("*:%s",   [local.region])
  key_any_any        = "*:*"
...
locals {
  _validate = selected_bucket != null ? true : tobool("No logging bucket found in registry for account " + local.account_id + " and region " + local.region)
}
